...
if len(sys.argv) < 2:
    csv_path = os.path.join(ROOT, "sessions_exported_for_paper.csv")
else:
    csv_path = sys.argv[1]
...
for g, sub in df.groupby("group"):
    n = len(sub)
    pre_m, pre_sd, _ = ci95_mean(sub["pre_score"])
    post_m, post_sd, _ = ci95_mean(sub["post_score"])
    gain_m, gain_sd, _ = ci95_mean(sub["gain"])
    rouge_m, rouge_sd, _ = ci95_mean(sub["rouge_l"])
    sat_m, sat_sd, _ = ci95_mean(sub["satisfaction"])
    lat_med = sub["ai_latency_ms"].median()
    lat_iqr = int(sub["ai_latency_ms"].quantile(0.75) - sub["ai_latency_ms"].quantile(0.25))
    groups.append({
        "group": g,
        "N": n,
        ...
        "ai_latency_iqr": lat_iqr if not pd.isna(lat_iqr) else "",
...
axes[0].set_title("Mean Learning Gain (post - pre)"); axes[0].set_ylabel("Score gain")
sns.boxplot(x="group", y="ai_latency_ms", data=df, ax=axes[1])
axes[1].set_title("AI Response Latency by Group (ms)"); axes[1].set_ylabel("Latency (ms)")
sns.barplot(x="group", y="rouge_l", data=df, ci=95, capsize=0.12, ax=axes[2])
axes[2].set_title("ROUGE-L by Group"); axes[2].set_ylabel("ROUGE-L")
plt.tight_layout()
